...
ImageNet은 22,000개의 카테고리를 가지는 15백만개의 라벨링된 고해상도이미지 데이터셋이다. 이미지들은 웹을 통해 모았고, 사람손에 의해서 라벨링 되었다.
이미지 분류 대회인 ILSVRC는 2010년 부터 시작되었다. 이 대회에서는 ImageNet 데이터의 서브셋을 사용한다. 대략 1.2백만개의 학습 데이터, 50,000개의 검증 데이터, 150,000개의 테스트 데이터이다. ILSVRC-2010은 테스트 세트의 라벨을 이용할 수 있기 때문에 우리의 연구에 이 버전을 사용하였다. (뒤에 ILVRC-2012의 데이터세트에 적용한 결과도 있음. 이 버전의 테스트 데이터 세트의 라벨은 이용불가임) 
ImageNet에서 top-1 error와 top-5 error를 기술하는 것이 관습적인데 top-5 erro란 실제 라벨이 모델이 예측한 상위 5개의 라벨에 속하지 않은 비율값이다. 
...
두번째 증식 방법은 RGB채널의 픽셀 값에 변화를 주는 것이다. 전체 학습 데이터 셋의 RGB값에 PCA를 수행하는 것이다. PCA를 통해 구해진 고유벡터값과 고유값X랜덤 변수(평균이 0이고 분산이 0.1인 가우시안 분산에서)를 곱한값을 각 RGB픽셀에 더해준다. 이 방법으로 데이터 증식을 했을때 top-1 오류율을 1%줄이는데 도움이 됐다.

### 4.2 Dropout

![image](https://user-images.githubusercontent.com/77263283/151658873-d95054dc-f53f-46c3-ae28-e1c07ffe0d4a.png)

많은 학습기를 앙상브라는 것은 test 오류를 줄이는 방법인데 큰 학습기를 여러개 학습시키는것은 오랜 시간이 걸린다. 그래서 논문에서는 이것과 비슷한 효과를 가지는 드롭아웃을 사용했다. 드롭아웃이란 한 뉴런의 출력을 확률값에 따라 비활성화/활성화 하는 것이다. 출력이 비활성화되면 그 뉴런의 출력 값은 앞으로 전해지지 않고 역전파에 참여하지 않는다. 그래서 입력 값이 주어질 때, 학습기는 같은 가중치를 공유하지만 매번 다른 아키텍쳐를 학습하는 효과가 있다. 드롭아웃을 사용하면 뉴런들은 다른 특정 뉴런의 존재에 의존할 수 없기 때문에 뉴런들의 복잡한 co-adaptation을 줄일 수 있다. 그래서 뉴런들은 더 강력한 특징들을 학습할 수 있게 된다.

논문의 모델에서 두개의 완전 연결층에 이 드롭아웃을 적용하였다.

## 5. Details of learning

논문에서는 128의 배치 사이즈와 0.9의 momentum, 0.0005의 weight decay의 SGD를 사용하여 학습했다. 이때 작은 크기의 weight decay는 매우 중요했다. 단순히 규제를 위한것이 아니라 모델의 학습 오류를 줄여주었다. 

각 층의 가중치를 평균이 0이고 분산이 0.01인 가우시안 분포로 초기화 하였으며, 두번째, 네번째, 다섯번째 Conv층과 모든 완전 연결층의 bias를 1로 나머지는 0으로 초기화 했다.

그리고 모든층에 같은 learning rate를 사용하였으며 검증셋의 오류율이 더 나아지지 않을 때 learning rate를 10으로 나누어주었다. 

## 6. Result

![image](https://user-images.githubusercontent.com/77263283/151658879-dc50ebb8-c58c-40e3-9148-7dd52dc095f1.png)

ILSVRC-2010에서 이 모델은 Top-1,Top-5 오류율을 각각 37.%, 17.0% 달성하였다. 

ILSVRC-2012 대회에서도 이 모델은 좋은 성능을 보였다. 2012대회에서 테스트 데이터세트에 대한 레이블은 공개되지 않았기 때문에 검증 오류율과 테스트 오류율을 호환성있게 사용하겠다(두 오류율은 0.1%이상 다르지 않기 때문).

![image](https://user-images.githubusercontent.com/77263283/151658883-aefd217e-b2a3-46df-a137-6a2ffab84b0a.png)

이 논문에서 소개된 CNN,모델로 top-5 오류율을 18.2% 달성하였다. 다섯개의 비슷한 CNN모델의 예측값의 평균을 낸것의 오류율은 16.4%였다. *표시가 붙은 모델들은 ImageNet fall 2011 데이터로 pre train된 모델을 fin tuning하여 예측을 한 모델이다.

### 6.1 Qualitative Evaluations

![image](https://user-images.githubusercontent.com/77263283/151658891-a17dad3c-289b-429b-ba50-370bcfdf0463.png)

상호작용이 제한된 두개의 GPU에서 학습이 이루어 진다고 했다. 따라서 위의 그림에서 보면 GPU1에서는 color-agnostic인 특징들이, GPU2에서는 color-specific적은 특징들이 추출된다.

![image](https://user-images.githubusercontent.com/77263283/151658904-0bca92b5-cbd1-4c88-9bb0-edaf55d15caa.png)

여덟개 이미지에 대한 상위 5개의 예측값을 통해 이 모델의 학습을 평가할 수 있다. 첫 번째 사진처럼 사물이 중앙에서 벗어난 이미지도 잘 에측을 하였으며 거의 대부분의 상위 5개 예측은 합리적이다. 

네트워크의 시각적 지식을 조사하는 다른 방법은 마지막 4096차원의 은닉층의 이미지에서 유도된 특징 활성을 고려하는 것이다. 만약 두개의 이미지가 유클리디안 거리가 가까운 특징 활성 벡터를 만든다면 높은 수준의 네트워크 층은 이 이미지를 비슷하다고 생각할 것이다. 위 이미지의 오른쪽을 보면 첫번째 컬럼은 5개의 테스트 이미지이고 나머지 컬럼은 각 테스트 이미지와 가장 유클리디안 분리가 가장 비슷한 여섯개의 학습 이미지이다. 

## 7. Discussion

이 결과들은 크고 깊은 CNN,은 순수 지도학습을 사용해서 높은 수준의 어려운 데이터 세트에서 기록적은 달성을 보여준다. 만약 이 모델에서 하나의 Conv층이 제거된다면 성능 저하가 일어날 것이다. 따라서 네트워크의 깊이는 이 기록들을 달성하는 데 매우 중요하다. 

실험을 단순화하기 위해 pre traing된 모델을 사용하지 않았다. 더 크고 학습을 오래 시키면 결과도 개선되겠지만 인간의 시각 시스템을 따라가기엔 많은 과제가 있다.
