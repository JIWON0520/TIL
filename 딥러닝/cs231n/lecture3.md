...
이제 softmax loss를 더 잘 이해하기 위해 SVM loss에서 했던 동일한 질문을 해보자.

![사진19](https://user-images.githubusercontent.com/77263283/131852093-f958b691-1260-4aa5-b5d2-2147d0baca9b.png)

첫 번째 질문은 softmax loss의 최댓값과 최솟값은 무엇일까?

최솟값은 0일것이다. loss 값이 최소가 되기위한 최선의 상황은 정답 카테고리의 확률 분포가 1이 되는것이다. 손실 함수는 -logP이므로 -log1이다. -log1은 0이므로 최솟값은 0이된다. 하지만 정답 카테고리의 확률분포가 1이되려면 정답 카테고리의 스코어는 양의 무한대가 되어야하고 다른 카테고리 스코어는 음의 무한대가 되어야 한다. 왜냐하면 softmax loss는 원래 스코어에 지수를 취하고 또 정규화도 하기때문이다. 하지만 이렇게 스코어가 양의 무한대, 음의 무한대를 갖는것은 현실적으로 불가능하기 때문에 loss 값의 최소가 0이라는 것은 단지 이론상의 최솟값이다.

최댓값은 양의 무한대 값이다. softmax loss가 최대가 되려면 정답 카테고리의 확률분포가 0이 되어야한다. 그렇게 되면 -log0이 최댓값이 되는데, 이는 양의 무한대이다. 하지만 정답 카테고리가 0이 되려면 정답 카테고리 스코어에 지수를 취한 값이 0이 되어야 한다. 그럼 정답 카테고리 스코어는 음의 무한대가 되어야하는데 이것도 현실적으로 불가능하다. 따라서 loss 값의 최대가 양의 무한대라는것 또한 이론상의 최댓값이다.

![사진20](https://user-images.githubusercontent.com/77263283/131852127-d4a07bf6-c9d6-4735-8044-ceb9120fd2ce.png)

두 번째 질문은 만약 초기화된 W값이 매우 작아서 모든 스코어가 0에가까운 비슷한 값들로 나온다면 softmax loss값은 어떻게 될까? 

정답은 -log(1/C)=logC가 된다. 그 이유는 모든 스코어가 같다면 모두 같은 확률분포 1/C를 가질것이기 때문이다.

### **SVM loss VS softmax loss**

SVM loss와 softmax loss는 classifier의 나쁨을 평가하기 위한 score를 해석하는 방법이 다르다. SVM loss는 단지 정답 스코어와 다른 스코어들 사이의 '마진'에만 신경을 쓰기 때문에 정답 스코어가 다름 스코어보다 충분히 크다면 더이상 성능 개선에 신경쓰지 않는다. 반면에 softmax loss는 loss 값이 최대한 0에 가깝도록 끊임없이 성능개선을 할것이다. 정답 카테고리의 확률이 충분히 크더라도 그것이 1이 되도록 W를 수정할것이다. 위에서 말했듯이 정답 카테고리 확률이 1이 되기 위해서는 정답 카테고리 스코어가 양의 무한대가 되어야한다. 이는 불가능하지만 classifier는 계속 W를 업데이트 할것이다.

그렇다면 어떻게해서 loss값이 최소가 되게하는 W를 찾을 수 있을까?

우리는 Optimization을 통해서 W를 찾을 수 있다.

![사진21](https://user-images.githubusercontent.com/77263283/132325923-56f15bbc-b693-4427-8db3-10bd736a4270.png)

우리가 산위에 있는 사람이라고 생각해 보자. 우리는 산 정상에서 제일 낮은 곳으로 이동하고 싶지만 어디가 산 밑바닥인지 두 눈으로 정확히 볼 수 없을것이다. 이때 우리는 지리적 기하학을 이용하여 밑바닥으로 이동할 수 있다. 두 발로 어느 내려가는 경사를 따라 이동하다보면 우리는 산 맨 밑에 도착할 수 있을것이다.  이런 쉬운 알고리즘을 실제 학습에서 사용한다.

![사진22](https://user-images.githubusercontent.com/77263283/132325973-76b43d17-ace4-497a-8ebf-a12efc1b8ceb.png)

그렇다면 여기서 말하는 경사란 무엇일까?

일차원 함수에서 경사란 함수의 미분값을 뜻한다. 어느 점에서 미분값을 구하기 위해서는 어떤 작은 step h와  그 step간의 차이를 비교하고 그 step값을 0으로 근사시키면 우리는 그 점의 기울기(gradient)를 알 수 있다. 우리는 이 식을 다변수 함수로 확장시켜야 한다. 실제 딥러닝에서 x는 스칼라가 아닌 벡터이기때문에 이 gradient는 편미분 값들로 구성된 벡터이다.

gradient는 x와 같은 shape을 가질 것이고, 그 구성요소들은 우리가 그 방향으로 가면 함수의 경사가 어떤지 알려준다. gradient는 함수의 가장 높게 올라가는 방향을 제시해주고, gradient의 반대는 함수의 가장 많이 내려가는 방향을 알려준다. 많은 딥러닝에서는 함수의 gradient를 계산하고 이를 파라미터를 점차적으로 업데이트하는데 사용한다.

![사진23](https://user-images.githubusercontent.com/77263283/132326002-4df6451b-2ec2-4f40-9e90-b29b160b8755.png)

우리는 W를 어떠한 값으로 초기화 한 뒤, 손실 값과 gradient를 계산한다. 그리고 작은 step씩 gradient의 반대방향으로 W를 업데이트 할 것이다.  이 때 이 step은 매우 중여한 하이퍼파라미터이다. 이 하이퍼파라미터는 learning rate라고도 하는데, 우리가 W를 업데이트 할때마다 얼마큼 -gradient 방향으로 이동할지 정해준다.

![사진24](https://user-images.githubusercontent.com/77263283/132326020-6eea7266-24f9-4bc7-9802-5c021e27bfca.png)

여기 2차원예시가 있다. 위의 그래프에서 그릇처럼 보이는 것이 우리의 손실함수이다. 가운데의 빨간 지역이 우리가 원하는 loss값이 최소가되는 지점이고 끝쪽의 파란 영역과 초록영역이 loss값이 최대가 되는 지점이다. 우리는 임의의 값으로 W를 초기화하고 -gradient를 계산하여 W를 업데이트한다. 이를 계속 반복하면 우리는 결국 빨간 영역에 도달할 것이다.

![사진25](https://user-images.githubusercontent.com/77263283/132326041-2f846f10-1771-4c27-beb5-121d51582e38.png)

하지만 우리가 만약 학습하려는 데이터 세트가 매우 클 경우 이 샘플들의 loss와 gradient를 계산하는데 많은 시간이 걸릴 수 있다. 그래서 우리는 '확률적 경사하강법(SGD)'을 이용한다. SGD는 loss의 gradient를 계산하는데 모든 학습 세트를 사용하지 않고 'mini batch'라고 불리는 작은 사이즈의 샘플을 학습 데이터로 사용한다. classifier는 이 minibatch를 사용하여 loss의 추정값과 gradient의 추정값을 사용해서 W를 업데이트 한다.

### Feature Transform

![사진26](https://user-images.githubusercontent.com/77263283/132326077-5da0346d-f5ef-41fc-954a-269d74180526.png)

우리는 이제껏 linear classifier에 input을 줄때 날것의 픽셀 값을 주었다. 하지만 이것은 제대로 작동하지 않는다. deep neural network가 유행하기 전에 두가지 방법을 거쳐 input을 만들어냈다. 첫번째로 이미지 모양 같은 이미지 특징 표현을 계산하고 이것들을 하나의 벡터로 합쳐 linear classifier의 input으로 사용했다. 

위와 같은 예제가 있다고 하자. 왼쪽의 빨강,파랑 데이터들을 분리시켜주는 선을 그을 수 없다. 하지만 극좌표계를 사용하여 특징 변환을 하면 위의 빨강,파랑 점들은 선 하나로 구분할 수 있게 바뀌었다.

예제처럼 우리가 다루려는 문제에 알맞은 특징 변환을 찾고, 특징 변환을 이용한다면 날것의 픽셀값을 input값으로 사용하는 것보다 classifier는 더 잘 작동할 것이다.

![사진27](https://user-images.githubusercontent.com/77263283/132326096-e9412915-6a27-412d-b80e-b33b010f841e.png)

가장 단순한 특징 변환의 예는 'color histogram'이다. 컬러 스펙트럼을 각각의 구역을 나눠 bucket으로 만들고, 하나의 픽셀값마다 각 bucket을 맵핑하여 각 bucket에 얼마나 많은 픽셀이 있는지 센다. 그럼 그 이미지에 어떤 색깔이 많이 있는지 알려줄 것이다. 위의 개구리 사진에서 초록색의 값이 제일 많고, 보라색이나 빨간색은 거의 없음을 말해준다. 

![사진28](https://user-images.githubusercontent.com/77263283/132326115-b54dc87e-2a1a-49db-9f9a-050d12cf87c4.png)

또다른 특징 변환의 예는 'Histogram of Oriented Gradients(HOG)'이다. 이 방법은 이미지를 작은 픽셀로 나눠 각 픽셀에서 지배적인 edge를 계산하고 이를 histogram으로 만든것이다. 이는 이미지에 어떤 edge가 있는지를 알려준다. 

위의 개구리 사진을 보면 이파리 부분에는 대각선모양 edge가 많은 것을 볼 수 있다. 이 그림의 feaure transform을 시각화 한 histogram을 보면 이파리 부분에 대각선들이 많은 것이 보인다.

![사진29](https://user-images.githubusercontent.com/77263283/132326154-06299ddc-85d9-4c98-8e60-6360b0df9049.png)

다음 예시는 'Bag of Words'이다. 이 방법은 자연어 처리에서 영감을 얻은 방법이다. 이미지들을 수집한 다음 이 이미지 들을 임의로 조각내고 각 조각들을 군집화하면 각 군집은 이미지의 특징(색이라던가 edge라던가)을 표현하게 된다. 이러한 군집들을 'code book'이라고 한다. 하나의 이미지에서 코드북의 발생빈도를 통해 이미지를 인코딩 할수 있게된다. 그러면 이미지가 어떻게 생겼는지를 알 수 있다.
